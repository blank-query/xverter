...
``ZArchiveWriter``: 64 KiB blocks zstd-compressed at level 9 by default (stored raw
...
#: zstd level the writer uses when none is given: 9 (the reference writer
#: used 6). Rigs' ruling after the 2026-09-11/13 level sweeps: 19 is only
#: ~2% smaller than 9 on real game data and costs ~10x the build time, and
#: a library is written far more often than that 2% is worth. A block is
#: stored raw whenever compression does not shrink it, at any level, so
#: content is identical whichever level built it. Bounds: 1..22.
DEFAULT_LEVEL = 9
