...
**`.7z` input is read in place, natively.** Since 1.5.0 a `.7z` holding a disc image is not extracted first: xverter's own 7z reader (pure Python, stdlib LZMA) parses the archive and hands the image inside it straight to the writers, so `Halo 3.7z → .god` is one streamed pass with no unpacked copy on disk, and `probe`/`info` read the game's identity out of the archive without decompressing past the executable. Caveats, stated plainly: (1) it decodes folders compressed with **LZMA2, LZMA or Copy** - the shape every game rip in the wild has; a BCJ/BCJ2-filtered, PPMd, BZip2, Deflate or encrypted archive falls back to the 7-Zip engine and is extracted as before. (2) Random access exists only at **LZMA2 dictionary-reset boundaries**, which 7-Zip's multithreaded compressor (its default) writes about every four dictionaries; such an archive is read in place, blocks decoding independently and in parallel on any interpreter. An archive compressed single-threaded (or LZMA1, or with a dictionary larger than a quarter of the file) is one block with no entry points, and a conversion reads an image several times, so one of those is **decoded once into the scratch dir** (native, CRC-checked, then authenticated against redump like any ISO) and converted from there - the cost of an extraction, without the engine; only a plain `.7z → .iso` streams it straight through. `xverter info` on a `.7z` says which case it is. (3) Integrity: a decoded-once image and a `.7z → .iso` copy are checked against the archive's CRC like an extraction; a multithreaded archive read in place is read at random and cannot be, so there the output is verified by its own rules only, as always, and a corrupt LZMA stream fails the conversion outright. (4) Multi-volume archives and archives whose payload is not a disc image (a packed game folder, a CCI/CSO/CHD/zar inside a 7z) extract with the engine as before.
...
| `--level N` | zstd level for `.zar` output, 1–22 (default 6). Content is identical at any level — blocks that don't shrink are stored raw — only size and time change |
